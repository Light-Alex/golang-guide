...
| **映射（Map）**       | 哈希表（`map[K]V`）                             |
| **通道（Channel）**   | 用于 Goroutine 间通信的管道（`chan T`）         |
| **接口（Interface）** | 包含动态类型和值指针的抽象类型（`interface{}`） |
| **函数（Function）**  | 函数类型（`func`）                              |
...
- **指针本身是值类型**：存储的是另一个变量的内存地址。
- **通过指针操作数据**：可修改其指向的值（类似引用效果）。
...
- **字符串是值类型**：底层是只读的 `byte` 切片（`[]byte`），赋值时复制数据。
...
- **接口是引用类型**：接口变量存储动态类型信息和指向实际数据的指针。

```GO
var w io.Writer     // 接口类型
w = os.Stdout       // 存储 os.Stdout 的类型和指针
```
...
| **特性**      | **值类型**               | **引用类型**                           |
| ------------- | ------------------------ | -------------------------------------- |
| **内存分配**  | 变量直接存储数据，栈或堆 | 变量存储数据地址，数据在堆上           |
| **赋值/传参** | 复制整个值               | 复制引用（共享底层数据）               |
| **修改影响**  | 不影响原始变量           | 影响所有引用该数据的变量               |
| **典型类型**  | `int`、`array`、`struct` | `slice`、`map`、`channel`、`interface` |
...
在 Go 语言中，多返回值是通过 **栈传递** 和 **编译器自动生成结构体包装** 实现的。这种设计使得函数可以简洁地返回多个值（如结果和错误），同时保持高效的调用约定。以下是具体实现机制和示例：

**1. 返回值的内存布局**

当函数返回多个值时，编译器会将所有返回值 **打包成一个结构体**，通过 **栈（Stack）** 传递该结构体的地址。调用方从栈中读取该结构体的字段以获取返回值。
...
func foo() *foo_ret {
    ...
    ret.result0 = 42
    ret.result1 = "hello"
    return ret // 返回结构体指针（实际通过栈传递地址）
}
```

**2. 调用约定（Calling Convention）**

- **栈传递**：Go 的调用约定优先通过栈传递参数和返回值（而非寄存器），以简化跨平台实现。
- **隐式参数**：调用方会在栈上预留空间，函数将返回值写入该空间。

**3. 命名返回值（Named Return Values）**

命名返回值本质是 **预先在栈上分配的结构体字段**，函数内部直接操作这些字段：

```GO
func bar() (x int, y string) {
    x = 42      // 直接修改栈上的返回值结构体字段
    y = "hello"
    return      // 隐式返回 x 和 y
}
```
...
### Go中init 函数的特征?
答：一个包下可以有多个 init 函数，每个文件也可以有多个 init 函数。多个 init 函数按照它们的文件名顺序逐个初始化。应用初始化时初始化工作的**顺序是**，**从被导入的最深层包开始进行初始化，层层递出最后到 main 包**。**不管包被导入多少次，包内的 init 函数只会执行一次**。但**包级别变量的初始化先于包内 init 函数的执行。**
...
在 Go 语言中，`uintptr` 和 `unsafe.Pointer` 都是用于底层指针操作的机制，但它们的行为和用途有本质区别。以下是详细对比及使用场景说明：

**核心区别对比表**

| **特性**               | **`unsafe.Pointer`**                               | **`uintptr`**                                      |
| ---------------------- | -------------------------------------------------- | -------------------------------------------------- |
| **类型安全**           | 类型安全的指针抽象，可指向任意类型                 | 无类型信息的整数，仅存储地址值                     |
| **垃圾回收（GC）影响** | 持有 `unsafe.Pointer` 的对象会被 GC 识别并保持存活 | `uintptr` 是纯数值，GC 无法感知其关联的内存        |
| **指针运算**           | 不支持直接算术运算，需转换为 `uintptr` 再操作      | 支持整数运算（如地址加减）                         |
| **与 `nil` 的互操作**  | 可与 `nil` 比较（如 `ptr == nil`）                 | 无 `nil` 概念，0 值表示无效地址                    |
| **使用场景**           | 类型转换、访问非导出字段、与 C 指针交互            | 地址计算、系统调用（需配合 `unsafe.Pointer` 使用） |
...
- **定义**：通用的指针类型，可指向任意类型的变量。
- 核心用途：
  - **类型转换**：将一种指针类型转换为另一种。
  - **绕过类型系统**：访问结构体的非导出字段。
  - **与 C 指针交互**：在 CGO 中传递指针到 C 函数。
...
func main() {
    ...
    // 转换为 *int32 指针
    int32Ptr := (*int32)(ptr)
    fmt.Println(*int32Ptr) // 输出底层字节的 int32 值（可能非 42）
}
```

**2. `uintptr`**

- **定义**：足够大的整数类型，用于存储指针的位模式（非指针类型）。
- 核心用途：
  - **指针运算**：计算内存地址偏移量。
  - **系统调用**：传递地址的整数值（如 `syscall.Syscall`）。
  - **临时存储地址**：但需确保对象不被 GC 回收。
...
func main() {
    ...
    // 计算第二个元素的地址
    elemPtr := unsafe.Pointer(uintptr(ptr) + unsafe.Sizeof(arr))
    second := *(*int)(elemPtr)
    fmt.Println(second) // 输出 2
}
```
...
+ **<font style="color:rgb(5, 7, 59);background-color:rgb(253, 253, 254);">由</font>**`GOMAXPROCS`**决定**：P的实际数量由环境变量`GOMAXPROCS`（或在Go程序中通过`runtime.GOMAXPROCS`函数设置）决定。这个值限制了同时运行的goroutine的数量，即在任何给定时间，最多只有`GOMAXPROCS`<font style="color:rgb(5, 7, 59);background-color:rgb(253, 253, 254);">个goroutine在CPU上执行。</font>
...
+ **默认和最大限制**：Go程序启动时，会设置一个M的最大数量（默认通常是10000，但这个值可能因Go版本和操作系统而异），但这个限制很少达到，因为操作系统本身就有线程/进程数量的限制。此外，通过`runtime/debug`包中的`SetMaxThreads`<font style="color:rgb(5, 7, 59);background-color:rgb(253, 253, 254);">函数可以设置M的最大数量，但这个函数主要用于调试目的，不建议在生产环境中随意更改。</font>
